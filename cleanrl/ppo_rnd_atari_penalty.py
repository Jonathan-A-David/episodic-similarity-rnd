...
if __name__ == "__main__":
    ...
    episode_buffer = torch.zeros((args.num_envs, args.buffer_size, 512), device=device)
    buffer_idx = 0
    ...
    next_ob = np.zeros((args.num_steps * args.num_envs, 1, 84, 84), dtype=np.float32)
    idx = 0
    for step in range(args.num_steps * args.num_iterations_obs_norm_init):
        acs = np.random.randint(0, envs.single_action_space.n, size=(args.num_envs,))
        s, _, _, _, _ = envs.step(acs)
        # Assuming FrameStack with 4 frames, take the latest one (index 3)
        next_ob[idx:idx+args.num_envs] = s[:, 3, :, :].reshape([-1, 1, 84, 84])
        idx += args.num_envs

        if idx >= args.num_steps * args.num_envs:
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            rnd_next_obs = (
                (
                    (next_obs[:, 3, :, :].reshape(args.num_envs, 1, 84, 84) - torch.from_numpy(obs_rms.mean).float().to(device))
                    / torch.sqrt(torch.from_numpy(obs_rms.var).float().to(device))
                ).clip(-5, 5)
            ).float()
            ...
            if args.use_penalty:
                with torch.no_grad():
                    ...
                    rollout_penalty_ratio += penalty_mask.mean().item()
                    rollout_cos_sim += max_sim.mean().item()
        ...
        curiosity_rewards /= float(np.sqrt(reward_rms.var))
        ...
        rnd_next_obs = (
            (
                (b_obs[:, 3, :, :].reshape(-1, 1, 84, 84) - torch.from_numpy(obs_rms.mean).float().to(device))
                / torch.sqrt(torch.from_numpy(obs_rms.var).float().to(device))
            ).clip(-5, 5)
        ).float()
        ...
        if args.use_penalty:
            writer.add_scalar("charts/penalty_applied_ratio", rollout_penalty_ratio / args.num_steps, global_step)
            writer.add_scalar("charts/avg_max_cos_sim", rollout_cos_sim / args.num_steps, global_step)
