...
# OUTPUT DIRECTORIES
# ─────────────────────────────────────────────
results_dir = "results/tau_ablations"
threshold_dir = os.path.join(results_dir, "plots")
data_dir = os.path.join(results_dir, "raw_data")

for d in [results_dir, threshold_dir, data_dir]:
    os.makedirs(d, exist_ok=True)

# ─────────────────────────────────────────────
# INPUT RUN DIRECTORIES
# ─────────────────────────────────────────────
runs_dir = "runs_buffer50_tau"

if not os.path.exists(runs_dir):
    print(f"Error: {runs_dir} does not exist. Run the experiments first!")
    exit(1)
...
prefixes = {
    "b50_tau98": "NoisyTVKeyDoorGridWorldSmall-v0__b50_tau98__",
    "b50_tau95": "NoisyTVKeyDoorGridWorldSmall-v0__b50_tau95__",
    "b50_tau90": "NoisyTVKeyDoorGridWorldSmall-v0__b50_tau90__",
    "b50_tau85": "NoisyTVKeyDoorGridWorldSmall-v0__b50_tau85__",
    "b50_tau80": "NoisyTVKeyDoorGridWorldSmall-v0__b50_tau80__",
}
...
print("Detected Buffer50 Tau Ablation Paths:")
print(json.dumps(paths, indent=2))
print("Detected Buffer50 Tau CSV Paths:")
...
# Colour palette: cool sequential blues/purples to visualise the threshold sweep
colors = {
    "b50_tau98": "#6366F1",   # Indigo
    "b50_tau95": "#8B5CF6",   # Violet
    "b50_tau90": "#EC4899",   # Pink
    "b50_tau85": "#F59E0B",   # Amber
    "b50_tau80": "#EF4444",   # Red
}

names = {
    "b50_tau98": "ES-RND (K=50, Tau=0.98)",
    "b50_tau95": "ES-RND (K=50, Tau=0.95)",
    "b50_tau90": "ES-RND (K=50, Tau=0.90)",
    "b50_tau85": "ES-RND (K=50, Tau=0.85)",
    "b50_tau80": "ES-RND (K=50, Tau=0.80)",
}
...
stats_df.to_csv(os.path.join(data_dir, "buffer50_tau_stats.csv"), index=False)
with open(os.path.join(data_dir, "buffer50_tau_summary.json"), "w") as f:
    json.dump(summary_stats, f, indent=2)
print("Saved buffer50_tau_stats.csv and buffer50_tau_summary.json")
...
ax.set_title("Tau Ablation at Buffer Size K=50\nES-RND on 9×9 Noisy-TV Key-Door Gridworld (500k steps)", fontsize=15, weight="bold")
ax.legend(frameon=True, fontsize=11)
ax.set_ylim(bottom=-0.02)
plt.tight_layout()
plt.savefig(os.path.join(threshold_dir, "b50_tau_success_curves.png"), dpi=300, bbox_inches="tight")
plt.close()
print(f"Saved b50_tau_success_curves.png in {threshold_dir}")
...
ax.set_title("TV Room Captivity vs Similarity Threshold (K=50)", fontsize=15, weight="bold")
...
plt.tight_layout()
plt.savefig(os.path.join(threshold_dir, "b50_tau_tv_occupancy.png"), dpi=300, bbox_inches="tight")
plt.close()
print(f"Saved b50_tau_tv_occupancy.png in {threshold_dir}")
...
ax.set_title("Embedding Cosine Similarity vs Threshold (K=50)\nMeasures how quickly the buffer saturates", fontsize=15, weight="bold")
ax.legend(frameon=True, fontsize=11)
plt.tight_layout()
plt.savefig(os.path.join(threshold_dir, "b50_tau_cos_sim_curves.png"), dpi=300, bbox_inches="tight")
plt.close()
print(f"Saved b50_tau_cos_sim_curves.png in {threshold_dir}")
...
ax.set_title("Penalty Trigger Rate vs Threshold (K=50)\nHigher = more frames suppressed by boredom penalty", fontsize=15, weight="bold")
ax.legend(frameon=True, fontsize=11)
ax.set_ylim(0, 1.05)
plt.tight_layout()
plt.savefig(os.path.join(threshold_dir, "b50_tau_penalty_ratio.png"), dpi=300, bbox_inches="tight")
plt.close()
print(f"Saved b50_tau_penalty_ratio.png in {threshold_dir}")
...
ax.set_title("Episode Length vs Threshold (K=50)\nShorter = agent solving the task more efficiently", fontsize=15, weight="bold")
ax.legend(frameon=True, fontsize=11)
plt.tight_layout()
plt.savefig(os.path.join(threshold_dir, "b50_tau_episode_length.png"), dpi=300, bbox_inches="tight")
plt.close()
print(f"Saved b50_tau_episode_length.png in {threshold_dir}")
...
def plot_heatmap(key, dest_dir, filename, grid_size=9):
    ...
    try:
        ...
        # Create a modern, stunning, and vibrant custom linear colormap
        # It transitions from a beautiful crisp slate white (unvisited) to a gorgeous lavender/indigo
        # and deepens to an intense, professional royal navy (very high density).
        ...
        # Plot heatmap with soft white gridlines for maximum cleanliness
        ...
        cbar.outline.set_visible(False) # Remove colorbar frame for modern look
        
        # Format the axes for a clean publication frame
        ...
        def add_glowing_landmark(ax, x, y, label, border_color, text_color, bg_color, is_dashed=False):
            ...
            # Elegant text badge with crisp rounded border and high-contrast styling
        ...
        # Draw game landmarks with their high-end stylized badges
        ...
        # 3. DOOR (4,4) - center cell. row=4, col=4 (dashed border)
        ...
        # 6. Sleek, solid dark slate walls (row 0..3 and 5..8 in col 4, leaving door cell (4,4) open)
        # Using a solid modern grid block structure instead of retro hatched lines
        ...
        # 7. PERFORMANCE METRICS OVERLAY BOX (floating premium dashboard widget in bottom-right room)
...
for key in prefixes.keys():
    tau_label = key.replace("b50_", "")   # e.g. tau98
    plot_heatmap(key, threshold_dir, f"heatmap_{tau_label}.png")

# ─────────────────────────────────────────────
# SUMMARY TABLE (printed to stdout)
# ─────────────────────────────────────────────
print("\n" + "=" * 90)
print("BUFFER-K50 TAU ABLATION SUMMARY")
print("=" * 90)
...
brain_dir_1 = "/Users/jonathandavid/.gemini/antigravity/brain/54789afb-b042-40ad-aa5f-eb4bf44bc7ed/results/buffer50_tau_ablations"
brain_dir_2 = "/Users/jonathandavid/.gemini/antigravity/brain/54789afb-b042-40ad-aa5f-eb4bf44bc7ed/results_small_grid/buffer50_tau_ablations"
...
print("\n--- BUFFER50 TAU ABLATION GRAPH SUITE GENERATED SUCCESSFULLY UNDER results/tau_ablations/! ---")
